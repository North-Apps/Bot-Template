...
if (!member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) return interaction.reply({ content: "You can't demote this user.", ephemeral: true });
...
if(mode === "merged" || mode === "rank"){
  ...
  try {
    ...
    if (interaction.user.id == id) return interaction.reply({ content: "You can't demote yourself.", ephemeral: true });
